...
function createKeyboard(id) {
	...
	let opts = {};
	if (reactionIndex > -1) opts = reactionIds[reactionIndex];
	else {
	...
	let keyboard = emojis.map((e, i) => {
		if (opts["response"][i].length === 0) return { text: e, callback_data: `${id}-${i}` };
...
bot.onText(/\/broadcast (.+)/, async (msg, match) => {
	...
	for (let i = 0; i < chatIds.size; i += 1) {
...
async function fetchAPI() {
	try {
		...
		for (let c = 0; c < confessions_array.length; c += 1) {
			let msg = `${confessions_array[c]["text"]}\nhttps://fb.com/${confessions_array[c]["id"]}`;
			if (confessions_array[c]["id"] === null) msg = `${confessions_array[c]["text"]}\nhttps://nuswhispers.com/tag/${confessions_array[c]["cid"]}`;
			let msges = chunkSubstr(msg, 4050);
			let chatIdIterator = chatIds.values();
			for (let u = 0; u < chatIds.size; u += 1) {
				let chatId = chatIdIterator.next().value;
				if (msges.length > 1) {
					for (let m = 0; m < msges.length; m += 1) {
						await sleep(500);
